...
import { 
    collection, getDocs, updateDoc, doc, getDoc, addDoc, deleteDoc 
...
function approveRequest(requestId, resourceId) {
    ...
    updateDoc(resourceRef, { locked: false }) // Unlock resource
...
// ✅ Update resource locked/unlocked status
function updateResourceStatus(requestId, locked) {
    getDoc(doc(db, 'requests', requestId))
        .then(snapshot => {
            if (!snapshot.exists()) {
                console.error("Request not found.");
            ...
            const requestData = snapshot.data();
            const resourceRef = doc(db, 'resources', requestData.resourceId);

            updateDoc(resourceRef, {
                locked: locked
            }).then(() => {
                console.log(`Resource ${requestData.resourceId} updated: locked=${locked}`);
            }).catch(error => {
...
// ✅ Attach functions to `window` to fix the "function not defined" error
